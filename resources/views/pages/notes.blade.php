    ...
    @if(count($notes) > 0)
        @foreach($notes as $note)
            <div class="grid-item notes-list">
                <div class="card bg-light mb-3 {{ $note->active == 1 ? '' : 'not-active no-shadow' }}" style="max-width: 20rem;">
                  ...
                  <div class="card-body">
                    ...
                    <small class="additional-info {{ $note->active == 1 ? 'text-muted' : 'not-active' }}">Data: {{ isset($note->date) ? date('d-m-Y', strtotime($note->date)) : 'Brak' }}</small>
                    <small class="additional-info {{ $note->active == 1 ? 'text-muted' : 'not-active' }}">Przez: {{ $note->name }}</small>
                  </div>
                   <div class="card-footer">
                     @if($note->created_at == $note->updated_at)
                        <small class="{{ $note->active == 1 ? 'text-muted' : 'not-active' }} note-timestamps"><i class="fa fa-calendar" data-toggle="tooltip" data-placement="top" title="Data dodania"></i> {{ date('d-m-Y H:i:s', strtotime($note->created_at)) }}</small>
                     @else
                        <small class="{{ $note->active == 1 ? 'text-muted' : 'not-active' }} note-timestamps"><i class="fa fa-refresh" data-toggle="tooltip" data-placement="top" title="Ostatnia aktualizacja"></i> {{ date('d-m-Y H:i:s', strtotime($note->updated_at)) }}</small>
                     @endif
                   </div>
                   <div class="card-actions no-wrap">
                        @if($note->active == 1)
                            <a href="/notes/{{ $note->id }}/disable" class="btn btn-light disable-note" data-toggle="tooltip" data-placement="top" title="Zablokuj"><i class="fa fa-ban"></i></a>
                        ...
                        @else
                            <a href="/notes/{{ $note->id }}/enable" class="btn btn-light enable-note" data-toggle="tooltip" data-placement="top" title="Odblokuj"><i class="fa fa-unlock-alt"></i></a>
                        @endif
                   </div>
                </div>
            </div>
